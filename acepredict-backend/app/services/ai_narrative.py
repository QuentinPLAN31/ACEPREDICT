...
def _build_prompt(ctx: dict) -> str:
    ...
    lines = [
        ...
        "du pronostic. "
        f"Puis, séparé par une ligne vide, EXACTEMENT ces 11 sections dans "
        ...
        "### SCENARIOS -- exactement 3 puces, chacune un scénario tactique "
        ...
        "moment du match ça peut basculer). Les 3 scénarios doivent être "
        "nettement différents les uns des autres (ex: un scénario où le "
        "favori déroule sans accroc, un scénario plus disputé qui bascule "
        "sur un moment précis, un scénario de surprise crédible si "
        "l'outsider impose son style) -- jamais deux variantes du même "
        "scénario. "
        ...
        "les autres sections. "
        "### CONSEIL_TACTIQUE -- exactement 2 puces (max 22 mots chacune), "
        "une par joueur (commence chaque puce par le prénom+nom du joueur "
        "concerné) : la consigne tactique concrète qu'un coach lui "
        "donnerait pour CE match précis, fondée sur les styles de jeu, la "
        "surface ou le format du tournoi fournis ci-dessous -- si le style "
        "de jeu d'un joueur n'est pas fourni, fonde son conseil sur son "
        "niveau général, sa forme ou la surface plutôt que d'inventer un "
        "style. "
        "### CHIFFRE_CLE -- AUCUNE puce : une seule phrase courte (max 22 "
        "mots) qui isole, PARMI les données déjà fournies ci-dessous, le "
        "seul chiffre ou statistique le plus déterminant pour ce pronostic "
        "et explique en quelques mots pourquoi -- jamais un chiffre "
        "inventé, uniquement un de ceux donnés dans le contexte. "
    ...
    lines += [
        ...
        "### SCENARIOS (3 puces), ### VIGILANCE (3 à 4 puces), "
        "### CONSEIL_TACTIQUE (2 puces), ### CHIFFRE_CLE (1 phrase SANS "
        "puce), ### VERDICT_EXPERT (2-3 phrases SANS puce) -- rien de "
        "redondant avec les chiffres déjà donnés ci-dessus ni entre les "
        "sections elles-mêmes, aucun remplissage. Ne donne jamais de "
        "conseil de pari, de cote, ni de garantie de résultat.",
    ]
